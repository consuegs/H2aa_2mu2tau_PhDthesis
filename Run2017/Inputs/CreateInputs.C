...
void BDTClassificationSignal(TString mass = "15p0")
{
	...
	for (int icat = 0; icat < ntrackCat; icat++)	// Loop over regions
	{
		...
		reader->BookMVA("BDT", "/nfs/dust/cms/user/consuegs/H2aa_2mu2tau/Run2017/MVA_BDT/ma" + mass + "/weights/TMVAClassification_BDT" + trackCat[icat] + ".weights.xml");
...
void BDTClassificationData(TString mass = "15p0")
{
	...
	TFile *fileIn = new TFile("../SingleMuon_Run2017.root");
	TFile *fileInSS = new TFile("../SingleMuon_Run2017_SS.root");
...
void InputsDataCards(double mass = 15.0, TString trackCat = "_lep_had")
{
	...
	TString BaseName = "haa-13TeV_2017_ma" + mass_string;

	TFile *fileInputs = new TFile(DirName + BaseName + trackCat + ".root", "recreate");
	hist_obs->Write("data_obs");
	hist->Write("bkgd");
	histGGH->Write("ggh");
	histUncUp->Write("bkgd_CMS_bdtbkg" + trackCat + "_2017Up");
	histUncDown->Write("bkgd_CMS_bdtbkg" + trackCat + "_2017Down");
	...
	for (int iparameter = 0; iparameter < nParameters; iparameter++)	// Loop over parameters
	{
		...
		histGGHUncUp->Write("ggh_CMS_" + Parameters[iparameter] + trackCat + "_2017Up");
		histGGHUncDown->Write("ggh_CMS_" + Parameters[iparameter] + trackCat + "_2017Down");

		textFile << "CMS_" + Parameters[iparameter] + trackCat + "_2017            shape   1.00    -  " << std::endl;
	}	// End loop over parameters
        ...
        for (int iBin = 1; iBin <=hist->GetNbinsX(); iBin++)        // Loop over Bins
        {
                ...
                //histUp->Write("bkgd_CMS_Bin_" + binname + trackCat + "_2017Up");
                //histDown->Write("bkgd_CMS_Bin_" + binname + trackCat + "_2017Down");

                //textFile << "CMS_Bin_" + binname + trackCat + "_2017            shape    -    1.00 " << std::endl;
        }	// End loop over bins
