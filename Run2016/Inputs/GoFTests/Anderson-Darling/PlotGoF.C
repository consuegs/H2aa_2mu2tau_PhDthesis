...
void PlotGoF(TString massf = "5", TString trk = "_lep_had", bool showPValue = true, bool showADValue = true)
{
	...
	float ADarling = Histo->AndersonDarlingTest(Histo_obs, "t");
	cout << "Anderson-Darling test: " << ADarling << endl;
	...
	Histo->GetXaxis()->SetRangeUser(0., 80);
	Histo->SetStats(0);
	...
	Histo->GetXaxis()->SetTitle("AD test statistic");
	...
	if (showADValue)
	{
		TPaveLabel *ADTest = new TPaveLabel(0.6, 0.38, 0.85, 0.45, Form("AD. Test = %5.3f", ADarling), "NDC");
		ADTest->SetTextSize(0.45);
		ADTest->Draw("same");
	}
	...
	c->Update();
	c->Print("H2aa_2mu2tau_GoFma" + massf + trk + "_AD.pdf", "Portrait pdf");
	delete c;
