...
void PlotLimits(bool blindData = false, char *fileList = "limits")
{
	...
	for (int i = 0; i < counter; ++i)
	{
		...
		char strOut[200];
		sprintf(strOut, "%5.2f & %5.2f & %5.2f & %5.2f & %5.2f & %5.2f & %5.2f",
			mA[i], minus2[i]*10, minus1[i]*10, median[i]*10, plus1[i]*10, plus2[i]*10, obs[i]*10);
    ...
    latex.DrawLatex(3.6,1.01,"#times 10^{-3}");
