...
void PlotGoF(TString massf = "7", bool showPValue = true)
{

	gROOT->SetBatch();

	TFile *file_obs = new TFile("higgsCombineTest.GoodnessOfFit.mH" + massf + ".root");
	TFile *file_toys = new TFile("higgsCombineTest.GoodnessOfFit.mH" + massf + ".123456" + ".root");
	...
	Histo->GetXaxis()->SetTitle("#chi^{2}-estimator (saturated model)");
	...
	if (showPValue)
	{
		TPaveLabel *PValue = new TPaveLabel(0.6, 0.55, 0.85, 0.62, Form("P(#chi^{2} > obs) = %.2f", pvalue), "NDC");
		PValue->SetTextSize(0.45);
		PValue->Draw("same");
	}

	c->Update();
	c->Print("H2aa_2mu2tau_GoFma" + massf + ".pdf", "Portrait pdf");
	delete c;
