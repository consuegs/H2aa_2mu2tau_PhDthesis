...
int main(int argc, char *argv[])
{
	...
	const string MuonIsoEffFile = cfg.get<string> ("MuonIsoEffFile");
	...
	TH1D *muIdLWeightH = new TH1D("muIdLWeightH", "", 100, 0, 2);
	TH1D *muIdTWeightH = new TH1D("muIdTWeightH", "", 100, 0, 2);
	...
	TH1D *counter_FinalEventsH[nDRInterval][nTrackCat][nTrackCat];
	...
	for (int iInt = 0; iInt < nDRInterval; ++iInt)	/////starting loop over all Intervals (Categories) used for Histograms
	{
		...
		for (int jInt = 0; jInt < nTrackCat; ++jInt)	/////starting loop over track categories used for Histograms
		{
			...
			for (int kInt = 0; kInt < nTrackCat; ++kInt)	/////starting loop over track categories used for Histograms
			{
				...
				counter_FinalEventsH[iInt][jInt][kInt] = new TH1D("counter_FinalEventsH" + DRIntervalString[iInt] + TrackCatString[jInt] + TrackCatString[kInt], "", 1, 0., 2.);
	...
	//ScaleFactor *SF_muonIso = new ScaleFactor();
	//SF_muonIso->init_ScaleFactor(TString(cmsswBase) + "/src/" + TString(MuonIsoEffFile));
	//IsoMu24
	...
	while (fileList >> filen)
	{
		...
		for (int iCand = 0; iCand < numberOfCandidates; iCand++)
		{
			...
			if (!isData)
			{
				...
				// if (posMuon.size()==1&&negMuon.size()==1&&posPion.size()==1&&negPion.size()==1) {      	//std::cout << "H->aa->2muon2tau : " << std::endl;
			...
			if (!isData)
			{
				...
				muIdLWeight = SF_muonId->get_ScaleFactor(muon_pt[iAmumu1candidate], muon_eta[iAmumu1candidate]);
				muIdTWeight = SF_muonId->get_ScaleFactor(muon_pt[iAmumu2candidate], muon_eta[iAmumu2candidate]);
			...
			muIdLWeightH->Fill(muIdLWeight, 1.0);
			weight *= muIdLWeight;
			muIdTWeightH->Fill(muIdTWeight, 1.0);
			weight *= muIdTWeight;
			...
			for (int iInt = 0; iInt < nDRInterval; ++iInt)	/////starting loop over all Intervals (Categories) for Dimuon Pair
			{
				...
				for (unsigned int iTrk = 0; iTrk < track_count; ++iTrk)
				{
					...
					if (Atautau1candidateDiff.P() > 0.1 && Atautau2candidateDiff.P() > 0.1 && Amumu1candidateDiff.P() > 0.1 && Amumu2candidateDiff.P() > 0.1)	// track is not any of the selected objects
					{
						...
						if (drTrkAmumu1 < DRInterval[iInt])
						{
							...
							if (fabs(track_pt[iTrk]) < ptTrkCut) soft_trkAmumu1candidate.push_back(iTrk);
						...
						if (drTrkAmumu2 < DRInterval[iInt])
						{
							...
							if (fabs(track_pt[iTrk]) < ptTrkCut) soft_trkAmumu2candidate.push_back(iTrk);
						...
						if (drTrkATauTau1 < DRInterval[iInt])
						{
							...
							if (fabs(track_pt[iTrk]) < ptTrkCut) soft_trkAtautau1candidate.push_back(iTrk);
						...
						if (drTrkATauTau2 < DRInterval[iInt])
						{
							...
							if (fabs(track_pt[iTrk]) < ptTrkCut) soft_trkAtautau2candidate.push_back(iTrk);
				...
				if (signalRegion)
				{
					...
					counter_FinalEventsH[iInt][IDAtautau1candidate][IDAtautau2candidate]->Fill(1., weight);
				}
