...
void GetFittingParVal()
{
	...
	float Par_1[3] = { 0.0359759, 0.035969, 0.0357994 };
	float Par_2[3] = { 0.00535085, 0.00592237, 0.00711309 };
	float Par_3[3] = { 0.0454146, 0.046334, 0.0500165 };
	float Par_4[3] = { 0.0127597, 0.0133509, 0.012523 };
	...
	float Par_8[3] = { 0, 0.0264235, 0.216367 };
	...
	for (int icategory = 0; icategory < nCategories; icategory++)	// Loop over categories
	{
		...
		for (int isample = 0; isample < nSamples; isample++)	// Loop over samples
		{
			...
			RooRealVar width_V("width_V", "width_V", 0.00570302 *points[isample], 0.00570302 *points[isample] - 0.003, 0.00570302 *points[isample] + 0.003);
			RooRealVar sigma_V("sigma_V", "sigma_V", 0.00817994 *points[isample], 0.00817994 *points[isample] - 0.003, 0.00817994 *points[isample] + 0.003);
...
void Validation(TString mass = "10")
{
	...
	for (int icategory = 0; icategory < nCategories; icategory++)	// Loop over categories
	{
		...
		Signal_Model_MuMuTrkTrkMET_Mass_val->plotOn(frame_MuMuTrkTrkMET_Mass, LineColor(kRed));
		...
		TString names_MuMuTrkTrkMET_Mass[] = { "h_dataSR",
			...
			"Signal_Model_MuMuTrkTrkMET_Mass_val_Norm[MuMuTrkTrkMET_Mass]",
			"" };
		...
		TLegend *legend_MuMu_Mass = new TLegend(0.49, 0.60, 0.92, 0.88);
		...
		TLegend *legend_TrkTrk_DR = new TLegend(0.49, 0.60, 0.92, 0.88);
		...
		//// Legend for MuMu_DR	////
		TLegend *legend_MuMu_DR = new TLegend(0.49, 0.60, 0.92, 0.88);
