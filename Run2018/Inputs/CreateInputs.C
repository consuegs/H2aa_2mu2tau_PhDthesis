...
void BDTClassificationSignal(TString mass = "15p0")
{
	...
	for (int icat = 0; icat < ntrackCat; icat++)	// Loop over regions
	{
		...
		reader->BookMVA("BDT", "/nfs/dust/cms/user/consuegs/H2aa_2mu2tau/Run2018/MVA_BDT/ma" + mass + "/weights/TMVAClassification_BDT" + trackCat[icat] + ".weights.xml");
...
void BDTClassificationData(TString mass = "15p0")
{
	...
	TFile *fileIn = new TFile("../SingleMuon_Run2018.root");
	TFile *fileInSS = new TFile("../SingleMuon_Run2018_SS.root");
...
void InputsDataCards(double mass = 15.0, TString trackCat = "_lep_had")
{
    ...
    //if (trackCat == "_lep_lep") {BkgdModel = "Sel_SS"; BkgdModel_unc = "NN00";}
    //else if (trackCat == "_lep_had") {BkgdModel = "00NN"; BkgdModel_unc = "Sel_SS";}
    //else  {BkgdModel = "00NN"; BkgdModel_unc = "00SoftIso";}
	...
	TString BaseName = "haa-13TeV_2018_ma" + mass_string;

	TFile *fileInputs = new TFile(DirName + BaseName + trackCat + ".root", "recreate");
	hist_obs->Write("data_obs");
	hist->Write("bkgd");
	histGGH->Write("ggh");
	histUncUp->Write("bkgd_CMS_bdtbkg" + trackCat + "_2018Up");
	histUncDown->Write("bkgd_CMS_bdtbkg" + trackCat + "_2018Down");
    ...
    textFile << "rate      " <<
		...
		bkgNorm << std::endl;
    textFile << "-----------------------------" << std::endl;
    textFile << "CMS_lumi_2018                           lnN         1.025          -" << std::endl;
    textFile << "CMS_eff_m_2018                          lnN         1.04           -" << std::endl;
    textFile << "CMS_trkiso_2018                         lnN         1.12           -" << std::endl;
    textFile << "CMS_bdtbkg" + trackCat + "_2018        shape         -           1.00" << std::endl; 
    textFile << "QCDScale_ggH                lnN         1.046/0.933    -" << std::endl;
    textFile << "PDF_ggh                     lnN         1.032          -" << std::endl;
    textFile << "bkgNorm" + trackCat + "_2018  rateParam  haa  bkgd  1  [0.5,1.5]" << std::endl;
	...
	for (int iparameter = 0; iparameter < nParameters; iparameter++)	// Loop over parameters
	{
		...
		histGGHUncUp->Write("ggh_CMS_" + Parameters[iparameter] + trackCat + "_2018Up");
		histGGHUncDown->Write("ggh_CMS_" + Parameters[iparameter] + trackCat + "_2018Down");

		textFile << "CMS_" + Parameters[iparameter] + trackCat + "_2018            shape   1.00    -  " << std::endl;
	}	// End loop over parameters
        ...
        for (int iBin = 1; iBin <=hist->GetNbinsX(); iBin++)        // Loop over Bins
        {
                ...
                //histUp->Write("bkgd_CMS_Bin_" + binname + trackCat + "_2018Up");
                //histDown->Write("bkgd_CMS_Bin_" + binname + trackCat + "_2018Down");

                //textFile << "CMS_Bin_" + binname + trackCat + "_2018            shape    -    1.00 " << std::endl;
        }	// End loop over bins
