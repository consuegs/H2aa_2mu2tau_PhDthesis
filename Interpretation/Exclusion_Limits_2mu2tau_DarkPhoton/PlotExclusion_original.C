...
void PlotExclusion()
{
	...
	for (Int_t i = 0; i < ntuple->GetEntries(); i++)
	{
		...
		float BR_H_aa_2mu2tau = 2 *BR_a_ll * BR_a_ll;	//BR_aa floating 

		if (m_a < lowest_ma || m_a > highest_ma) continue;
		ipoint++;

		Y_Minus2 = gr_minus2->Eval(m_a, 0, "S") / (BR_H_aa_2mu2tau *1000);
		Y_Minus1 = gr_minus1->Eval(m_a, 0, "S") / (BR_H_aa_2mu2tau *1000);
		Y_Median = gr_median->Eval(m_a, 0, "S") / (BR_H_aa_2mu2tau *1000);
		Y_Plus1 = gr_plus1->Eval(m_a, 0, "S") / (BR_H_aa_2mu2tau *1000);
		Y_Plus2 = gr_plus2->Eval(m_a, 0, "S") / (BR_H_aa_2mu2tau *1000);
		Y_Obs = gr_obs->Eval(m_a, 0, "S") / (BR_H_aa_2mu2tau *1000);
